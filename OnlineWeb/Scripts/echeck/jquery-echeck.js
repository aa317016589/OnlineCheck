 ...
 * @cfg [data-check.int] 整数
 * @cfg [data-check.mobile] 手机号
 * @cfg [data-check.max] 最大值
 * @cfg [data-check.min] 最小值
...
;(function($) {
    ...
    var defaults = {
        cls: {
            ...
            tipCls: 'form-tip',
            //错误提示样式
            errorTipCls: 'tip-error',
            //控件错误样式
            errorControlCls: 'control-error',
            //下拉列表样式
    ...
    var checkMethods ={
        ...
        isInteger: function (val) {
            return Math.floor(val).toString() === val;
        },
        /*是否为手机号码*/
        ...
        for (var i = 0, len = checkRules.length; i < len; i++) {
            ...
            if(checkValidate[rules.name]){
                ...
                if(rules.name === 'custom'){
                    ...
                    result = checkValidate[rules.name]($this.val(), rules.value, $this);
                }
